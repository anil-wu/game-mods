...
## §0 复刻宪法（最高原则，所有设计/开发任务必须附上并遵守）

> **所有系统、玩法和表现必须原原本本复刻（与原版一致）。**
>
> 1. **系统**：生命/体力/弹药/敌人/分数/道具等逻辑与数值与原版一致——不改数值、不简化机制；
> 2. **玩法**：移动/射击/AI/生成/拾取/计分等行为与原版一致；
> 3. **表现**：模型/特效/动画/UI/镜头/音效用原版资源（原 prefab/材质/贴图/动画控制器），
>    **禁止自建胶囊/方块/裸 ParticleSystem 等近似物代替**；仅原版资源确实缺失时才允许回退（需日志声明）。
>
> 与 Rule 12/13/14（跨 Mod 隔离/二进制/无泛型）冲突时，复刻宪法优先，但不得破坏这些通信红线。
